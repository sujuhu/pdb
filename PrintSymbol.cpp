...
//
void PrintPublicSymbol(IDiaSymbol *pSymbol, pdb_public_t* public)
{
  DWORD dwSymTag;
  DWORD dwRVA;
  DWORD dwSeg;
  ...
  if (dwSymTag == SymTagThunk) {
    if (pSymbol->get_name(&bstrName) == S_OK) {
      wprintf(L"%s\n", bstrName);
      wcsncpy(public->name, bstrName, NAME_SIZE - 1);
...
//
void PrintGlobalSymbol(IDiaSymbol *pSymbol, pdb_global_t* global)
{
  DWORD dwSymTag;
  DWORD dwRVA;
  DWORD dwSeg;
...
//
void PrintSymbol(IDiaSymbol *pSymbol, DWORD dwIndent, pdb_symbol_t* symbol)
{
  IDiaSymbol *pType;
  DWORD dwSymTag;
  ULONGLONG ulLen;
...
//
void PrintTypeInDetail(IDiaSymbol *pSymbol, DWORD dwIndent, pdb_udt_t* udt)
{
  IDiaEnumSymbols *pEnumChildren;
  IDiaSymbol *pType;
  IDiaSymbol *pChild;
...
////////////////////////////////////////////////////////////
//
void PrintSourceFile(IDiaSourceFile *pSource, pdb_file_t* pdb_file)
{
  BSTR bstrSourceName;
...
////////////////////////////////////////////////////////////
//
void PrintLines(IDiaSession *pSession, IDiaSymbol *pFunction, pdb_line_t* line)
{
  DWORD dwSymTag;
...
void PrintSecContribs(IDiaSectionContrib *pSegment, 
    pdb_section_contrib_t* pdb_contrib)
{
  DWORD dwRVA;
  DWORD dwSect;
  DWORD dwOffset;
...
//
void PrintFrameData(IDiaFrameData *pFrameData, pdb_fpo_t* fpo)
{
  DWORD dwSect;
  DWORD dwOffset;
  DWORD cbBlock;
